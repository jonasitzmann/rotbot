...
def main():
    ...
    for key in 'type start name location'.split():
        participation[key] = participation['url'].apply(lambda url: getattr(url2event[url], key))
    ...
    ax.axvspan(*challenge_dates, ymin=0, ymax=50, alpha=1, color='gray', label='challenge', zorder=0)
    half_rate = 14
    ...
    for i, training in df.iterrows():
        ...
        if len(context) == 1: # add np.nan as gaps in the plots where there is no context
            date_to_num_people_per_prob[start - datetime.timedelta(days=1)] = {1: np.nan}
        time_difference = np.array([(start - x['start']).days for i, x in context.iterrows()])
        ...
        rel_participations:np.array = ((context_participation*weights[:, None]).sum(axis=0))
        sorted_unique = sorted(set(rel_participations), reverse=True)
        num_people_per_prob = {}
        previous_num_people = 0
        for prob in sorted_unique:
            num_people = len([person for person in rel_participations if person >= prob])
            num_people_per_prob[prob] = num_people
            color = cmap(prob)
            if prob > 0:
                ax.plot([x, x], [previous_num_people, num_people], color=color, lw=2, zorder=0)
    ...
    for event_type, color in [(EventType.TOURNAMENT, 'blue'), (EventType.GAME, 'green'), (EventType.EVENT, 'black')]:
        df = participation[participation['type'] == event_type]
        ax.scatter(df['start'], np.zeros(len(df)) - 1, color=color, label=event_type.name.lower(), marker='^')
    # draw lines for concrete participations
    for prob in [0.25, 0.5, 0.75, 0.95]:
        darkness = 0.3
        color = np.array(cmap(prob)) * (1-darkness) + np.array([0, 0, 0, 1]) * darkness
        num_people_list = [(d[min([p for p in list(d.keys()) if p >= prob])]) for d in date_to_num_people_per_prob.values()]
        ax.plot(date_to_num_people_per_prob.keys(), num_people_list, color=color, label=prob, lw=2)
    ...
    ax.set_xlabel(f'Datum (Betrachtet wird EMA der letzten 30 Tage mit Halbierung alle {half_rate} Tage)')
...
def get_participation(download=False, start_date=(2019, 1, 1)):
    if download:
        url2event, participation = update_participation(future_weeks=0, update_async=False, start_date=start_date)
