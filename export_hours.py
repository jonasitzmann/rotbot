...
def main(starting_date=None):
    ...
    url2event, participation = get_participation(
        download=download,
        start_date=(starting_date.year, starting_date.month, starting_date.day),
    )
    ...
    for url, e in url2event.items():
        if e.start >= starting_date and e.type == EventType.TRAINING:
            ...
            num_participants = sum(
                (
                    participation[participation.url == url]
                    == Participation.YES.name.lower()
                ).values[0]
            )
            duration = (e.end - e.start).total_seconds() / 3600.0
            trainings.append(
                dict(
                    Datum=e.start.strftime("%d.%m.%y"),
                    Ort=location,
                    Uhrzeit=time,
                    Dauer=duration,
